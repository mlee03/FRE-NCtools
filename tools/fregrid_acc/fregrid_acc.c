...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  if( opcode & BILINEAR ) {  /* bilinear interpolation from cubic to lalon */
    ...
    setup_bilinear_interp_acc(ntiles_in, grid_in, ntiles_out, grid_out, interp_acc, opcode,
                              dlon_in, dlat_in, lonbegin_in, latbegin_in );
  }
  ...
  for(m=0; m<file_in->nt; m++) {
    ...
    for(l=0; l<nscalar; l++) {
      ...
      for(level_n =0; level_n < scalar_in->var[l].nn; level_n++) {
        if(extrapolate) {
          ...
          if( opcode & BILINEAR )
            do_scalar_bilinear_interp_acc(interp_acc, l, ntiles_in, grid_in, grid_out, scalar_in, scalar_out,
          ...
          for(level_z=scalar_in->var[l].kstart; level_z <= scalar_in->var[l].kend; level_z++) {
            ...
            if(debug) time_start = clock();
            if( opcode & BILINEAR )
              do_scalar_bilinear_interp_acc(interp_acc, l, ntiles_in, grid_in, grid_out,
    ...
    for(l=0; l<nvector; l++) {
      ...
      if( opcode & BILINEAR )
        do_vector_bilinear_interp_acc(interp_acc, l, ntiles_in, grid_in, ntiles_out, grid_out,
                                      u_in, v_in, u_out, v_out, finer_step, fill_missing);
      else
